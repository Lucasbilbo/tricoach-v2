...
function supabasePatch(path, key, body) {
  ...
  return new Promise((resolve) => {
    ...
    const req = https.request(options, (r) => {
      let d = '';
      r.on('data', chunk => d += chunk);
      r.on('end', () => { try { resolve(JSON.parse(d)); } catch { resolve(r.statusCode); } });
    });
    req.on('error', () => resolve(null));
    req.write(bodyStr);
    req.end();
...
// ─── Macrocycle logic (inline — mirrors create-cycle.js) ────────────────────
...
function calcularFases(fechaInicio, fechaFin) {
  ...
  if (totalSemanas <= 6) {
    return [
      { nombre: 'peak',  sem_inicio: 1, sem_fin: Math.max(1, totalSemanas - 2) },
      { nombre: 'taper', sem_inicio: Math.max(2, totalSemanas - 1), sem_fin: totalSemanas },
    ].map(withObj);
  }
...
exports.handler = async (event) => {
  ...
  if (!cycle) {
    return { statusCode: 404, headers: CORS, body: JSON.stringify({ error: 'No hay ciclo activo' }) };
  }

  // 2. Obtener perfil para leer carreras
  const profiles = await supabaseGet(
    `profiles?id=eq.${encodeURIComponent(userId)}&select=carreras,fecha_carrera,objetivo`,
    key
  );
  ...
  // 3. Determinar nueva carrera objetivo (A > sin prioridad > primera futura)
  ...
  // Fallback: profile.fecha_carrera
  ...
  // 4. Recalcular desde la semana actual
  const fechaInicio   = getMondayOfCurrentWeek();
  const fechaFin      = fechaCarrera || addWeeks(fechaInicio, 16);
  const semanasTotales = Math.max(1, Math.round(
    (new Date(fechaFin) - new Date(fechaInicio)) / (7 * 24 * 60 * 60 * 1000)
  ));
  const fases = calcularFases(fechaInicio, fechaFin);

  // 5. UPDATE training_cycles (mantener fecha_inicio original del ciclo)
  const updated = await supabasePatch(
    `training_cycles?id=eq.${encodeURIComponent(cycle.id)}`,
    key,
    {
      fases,
  ...
  const updatedCycle = Array.isArray(updated) ? updated[0] : null;
  const cicloFinal = updatedCycle || { ...cycle, fases, semanas_totales: semanasTotales, fecha_carrera: fechaCarrera, carrera_nombre: carreraNombre, carrera_tipo: carreraTipo };

  return {
    statusCode: 200,
    headers: { ...CORS, 'Content-Type': 'application/json' },
    body: JSON.stringify({ cycle: cicloFinal }),
  };
};
